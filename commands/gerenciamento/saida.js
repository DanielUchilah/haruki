    ...
    run: async(client, message, args, owner, database) => {
        ...
        var embed = {
            ...
            fields: [
                {
                    name: "📎 ⋅ Canal",
                    value: "Defina o canal que deseja que apareça a mensagem de saida. Eu aceito menção ou ID. **Utilize h/saida canal (Canal)**",
                    inline: true
                },
                {
                    name: "🎈 ⋅ Mensagem",
                    value: "Defina a mensagem de saida que deseja que apareça no canal definido. (user) para pegar a tag do usuário, (user.nome) para pegar o nome do usuário e (membros) para pegar a quantidade de membros do servidor. **Utilize h/saida mensagem (Mensagem)**",
                    inline: true
                },
                {
                    name: "<:equipe:745059485139075203> ⋅ Status",
                    value: "Desative o canal de saida utilizando off/on. **Utilize h/saida status on/off**",
                    inline: true
                }
            ]
        }
        ...
        switch(args[0].toLowerCase()) {
            ...
            case "canal":
                ...
                if(!canal) return message.channel.send("<:no:745406631277494323> ⋅ Você não inseriu o canal que deseja que apareça as mensagens de saida.")

                database.ref(`Servidores/Saida/${message.guild.id}`).once('value').then(async function (snap) {  
                  
                    if(snap.val() === null) {
                        
                        database.ref(`Servidores/Saida/${message.guild.id}`).set({
                            texto: 'Está é uma mensagem padrão, ou seja, não definiram a mensagem de saida ainda... Por favor, altere utilizando o comando **h/saida mensagem (mensagem)**',
                            canal: canal.id
                        })
                        ...
                        database.ref(`Servidores/Saida/${message.guild.id}`).update({
                            canal: canal.id
                        })
                        message.channel.send("<:yes:745409812212220064> ⋅ O canal foi alterado com sucesso.")
            ...
            case "mensagem":
                ...
                if(!argumento) return message.channel.send(`<:no:745406631277494323> ⋅ Você não inseriu a mensagem que deseja de saida... Ah, e lembrando, você tem alguns placeholders para colocar na mensagem. Por exemplo {user} mostra a tag do usuário. A baixo irei deixar alguns exemplos desses placeholders.\n\n**{user}**/${message.author.tag} - Tag do usuário.\n**{user.name}**/${message.author.username} - Username do usuário.\n**{membros}**/${message.guild.memberCount} - Membros atual do servidor.`)

                let placeholder1 = argumento.replace(/{user}/g, message.author.tag)
